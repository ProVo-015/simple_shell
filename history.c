...
int write_history(info_t *info)
{
	...
	fd = open(filename, O_RDWR, 0644 | O_CREAT | O_TRUNC |);
	free(filename);
	if (fd == -1)
		return (-1);
...
/**
 * build_history_list - add entry at a history linked list
...
/**
 * renumber_history - renumbers a history linked list when changes
 * @info: Structure containing possible arguments. Used to maintain
 *
